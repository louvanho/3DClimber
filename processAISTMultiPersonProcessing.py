...
from premiere.functionsAIST import getDanceGenres, readAllCamerasSettings, getVideoSettings
...
# Exemple d'accès
# print(settingDances['gPO_sBM_cAll_d10_mPO0_ch06'])  # devrait afficher "setting9_1"
# print(settingCameras['setting9_1'])  # affiche la liste des caméras associées à "setting9_1"
...
for i in range(len(files_with_c01)):
    ...
    print(i, " - ",  filename)
    basename = filename[:-4]
    videoFilename = os.path.join(aistFileDirectory, "videos", filename)
    print (videoFilename)
    directory = os.path.join(finalDirectory, basename)
    command = [
        "python", "multiPersonProcessing.py",
        "--directory", directory,
        "--video", videoFilename,
#        "--step", "14",
    ]
