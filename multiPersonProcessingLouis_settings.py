...
if __name__ == "__main__":
    ...
    parser.add_argument("--cam_nrs", type=str, default='c01,c02,c03,c04,c05,c06,c07,c08,c09', help="Camera numbers to process")
    ...
    cam_nrs = args.cam_nrs.split(",")

    if len(cam_nrs) != len(videoFileNames):
        print("Number of cameras and videos do not match")
        sys.exit(1)
    ...
    with open(camera_file) as f:
        data = json.load(f)
        focals = []
        for cam_nr in cam_nrs.split(","):
            for i in range(len(data)):
                if data[i]['name'] == cam_nr:
                    focals.append(np.array(data[i]['matrix'][0][0]))
                    break
            else:
                print(f"Camera {cam_nr} not found in camera settings file.")
                sys.exit(1)
