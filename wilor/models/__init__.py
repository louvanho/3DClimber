...
def load_wilor(checkpoint_path, cfg_path):
    ...
    if ('DATA_DIR' in model_cfg.MANO):
        ...
        model_cfg.defrost()
        model_cfg.MANO.DATA_DIR    = os.path.join(models_path,'mano')
        model_cfg.MANO.MODEL_PATH  = os.path.join(models_path,'mano')
        model_cfg.MANO.MEAN_PARAMS = os.path.join(models_path,'mano', 'mano_mean_params.npz')
        model_cfg.freeze()
