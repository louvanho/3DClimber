...
if __name__ == "__main__":
    ...
    videoFileNames = args.videos.split(",")
    for i in range(len(videoFileNames)):
        videoFileNames[i] = "\"" + videoFileNames[i] + "\""
    cam_nrs = ['c%02d' % (int(i)) for i in range(1, len(videoFileNames)+1)]
    print("Camera numbers: ", cam_nrs)
...
###################################################################################
    ...
    # for i in range(0):
        ...
        print ("\n############################################################")
        print ("# View nr", cam_nr)
        print ("############################################################")

        print ("\n############################################################")
        print ("# Video information")
        print ("############################################################")
        video = cv2.VideoCapture(args.videos.split(",")[i])
        ...
        if success:
            imgs_folder = os.path.join(args.directory, "imgs")
            if not os.path.exists(imgs_folder):
                os.makedirs(imgs_folder)
            output_image_path = os.path.join(imgs_folder, f"frame_{cam_nr}.png")
            cv2.imwrite(output_image_path, frame)
            print(f"Saved first frame of video {videoFileName} to {output_image_path}")
        else:
            print(f"Failed to read the first frame of video {videoFileName}")
        video.release()
        ...
        print ("############################################################")
        output_moge_pkl = os.path.join(args.directory, "moge"+cam_nr+".pkl")
        if args.step <= 0:
            print()
            output_moge_video = os.path.join(args.directory, "videoDepthAnalysis"+cam_nr+".mp4")
        ...
        if type == "nlf":
            ...
            print ("############################################################")
            output_type_pkl = os.path.join(args.directory, type+cam_nr+".pkl")
            output_type_pkls.append(output_type_pkl)
            if args.step <= 1:
                print()
    ...
    if args.step <= 1:
        print()
        image_folder = os.path.join(args.directory, "imgs")
        command_cameraEstimation = "python cameraEstimation.py --imgdir " + image_folder + " --outdir " + args.directory
        print("Processing camera estimation...")
        print(command_cameraEstimation)
        result = subprocess.run(command_cameraEstimation, shell=True)
        if result.returncode != 0:
            print("\nError in camera estimation")
            sys.exit(1)

    camera_file= os.path.join(args.directory, "camera_settings.json")

    print ("\n############################################################")
    print ("# Merge all views")
    print ("############################################################")
    output_type_pkl_merged = os.path.join(args.directory, type+"-merged.pkl")
    if args.step <= 1:
        print()
        pose_pkl_list = ""
        for i in range(len(output_type_pkls)):
...
###################################################################################
    ...
    print()
    # Open the pkl file
    print ("Read pkl: ",output_type_pkl_merged)
    file = open(output_type_pkl_merged, 'rb')
    ...
    if args.step <= 2:
        print()
        command_cleanFramesPkl = "python cleanFramesPkl.py " + output_type_pkl_merged + " " + output_cleaned_pkl + " " + str(humanNumber) + " " + str(threshold)
        ...
        if result.returncode != 0:
            ...
            sys.exit(1)
        print()
        command_injectCameraPropertiesPkl = "python injectCameraPropertiesPkl.py " + output_video_json + " " + output_type_pkl_merged + " " + output_type_pkl_merged
        print("Inject camera properties in", output_type_pkl_merged)
