...
def project_pose(points_3d, camera):
    ...
    translation_vector = torch.tensor(camera['translation'], dtype=torch.float32, device=device)  # (3,)
...
def compute_unary_from_2d(grid, joints_2d, cameras):
    """
    Compute unary terms based on 2D joint locations.

    Args:
        grid: 3D grid of possible joint locations (nbins^3, 3)
        joints_2d: List of 2D joint locations for each view (n_views, 2)
        cameras: Camera parameters for each view (intrinsics, extrinsics)
    Returns:
        unary: Unary term for each grid point (nbins^3,)
    """
    n_views = len(joints_2d)
    ...
    for v in range(n_views):
        ...
        # visualize_projection(projected_points, joints_2d, v)

        # Compute distance between projected points and 2D joint locations
        joints_2d_tensor = torch.tensor(joints_2d[v], dtype=torch.float32, device=grid.device)
        distances = torch.norm(projected_points - joints_2d_tensor, dim=1)
        unary += torch.exp(-distances / distances.mean())  # Normalize distances
...
def visualize_3D(pose3d):
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    ax.scatter(pose3d[:, 0].cpu(), pose3d[:, 1].cpu(), pose3d[:, 2].cpu())
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
...
if __name__ == '__main__':
    ...
    grid_size = 2  # Initial grid size
    fst_nbins = 16   # Initial number of bins in each dimension
    rec_nbins = 2    # Number of bins for recursive refinement
    tolerance = 150   # Tolerance for limb length constraints in mm
    rec_depth = 10    # Number of recursive refinement steps
    body = HumanBody()
    ...
    for framenr, frame in enumerate(file['allFrameHumans']):
        ...
        file['allFrameHumans'][framenr][0]['j3d_smplx'] = pose3d.detach().cpu().numpy()
        pbar.update(1)
    pbar.close()
