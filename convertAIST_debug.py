...
if __name__ == "__main__":
    ...
    for i in range(frames_count):
        ...
        # # Plot the original pose in 3D
        # fig = plt.figure()
        # ax = fig.add_subplot(111, projection='3d')
        # full_pose_rot_vec_tensor_cpu = full_pose_rot_vec_tensor.cpu()
        # ax.scatter(AISTPklJ3D['keypoints3d'][i, :, 0] / scaling, AISTPklJ3D['keypoints3d'][i, :, 1] / scaling, AISTPklJ3D['keypoints3d'][i, :, 2] / scaling, c='r', marker='o')
        ...
        #Manually correct the pose
        human['j3d_smplx'][:, 0] += -0.02
        human['j3d_smplx'][:, 1] += -0.13
        human['j3d_smplx'][:, 2] += -0.1

        # # Plot the corrected pose in 3D
        # ax.scatter(human['j3d_smplx'][:, 0], human['j3d_smplx'][:, 1], human['j3d_smplx'][:, 2], c='b', marker='o')
        # plt.show()


        R_np , _ = cv2.Rodrigues(camera_rotation)
        human['j3d_smplx'] = (R_np @ (human['j3d_smplx']).T).T + camera_position
        rotated_COCO = (R_np @ (AISTPklJ3D['keypoints3d'][i]/scaling).T).T + camera_position

        # # Plot both the original and corrected poses in 3D
        # fig2 = plt.figure()
        # ax2 = fig2.add_subplot(111, projection='3d')
        # ax2.scatter(rotated_COCO[:, 0], rotated_COCO[:, 1], rotated_COCO[:, 2], c='r', marker='o')
        # ax2.scatter(human['j3d_smplx'][:, 0], human['j3d_smplx'][:, 1], human['j3d_smplx'][:, 2], c='b', marker='o')
